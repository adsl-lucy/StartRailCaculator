# StartRailCaculator
蒙特卡罗方法求解：卡池出货分布/抽卡结果分布/有效圣遗物出货概率
...
除了第一个界面使用直接计算的方法，其余三个界面均采用蒙特卡罗的模拟方案。因为后三个界面对应的计算事件次数都是有限的，依照大数定律，重复足够多次实验后，事件重复频率趋于事件概率。在多次试验下，发现模拟次数为十万次时，计算速率和精度均足以满足实用需求。所以后三个界面的模拟次数均是十万次。
...
实际模拟发现
	...
	获取一个UP角色的抽数期望为104.7，对应星琼为16752.70
...
这种损失增大的效应在于：资源有限 + 分散投资 会同时增大了两个卡池的失败可能。所以对于资源有限的学生党，建议是：**按照期望规划，攒到保底入池，集中资源到少数关键角色/武器**。依照多次模拟的结论，按**先角色再武器**的抽卡次序，大致在220抽-230抽的情况下，UP武器沉船的概率是25%。210抽的情况UP武器沉船的概率是30，所以**大致攒33600-36800入池子是一个相对保守的方案**。
...
可见小月卡/日常+模拟宇宙/活动+十连这三者的收益非常接近，所以：**氪不氪小月卡取决是否希望每10个月多抽1个角色+专武**
...
其中获得成本最高的就是双爆衣和速度鞋，按照一天6次，一周7天来看，差不多普通副本主属性集齐需要一周，模拟宇宙部位集齐需要一周。考虑到实际情况下刷本的副产物还能给别的角色，实际一个角色主属性毕业差不多就1周时间。
...
![13 圣遗物概率分布](https://github.com/adsl-lucy/StartRailCaculator/assets/42862216/598cb27e-3eea-426a-b428-6658824e005c)
